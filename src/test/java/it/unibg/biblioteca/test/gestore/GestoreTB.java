...
public class GestoreTB {
	...
	@Test
	public void gestoreCancellaRichiestaTB() {
		getMapUtenti();
		...
		gestioneCatalogo = spy(gestioneCatalogo);
		...
		//doReturn(libro).when(gestioneCatalogo).cercaISBN(any());
		assertTrue(g.gestoreCancellaRichiesta(libro));
	}
	
	@Test
	public void gestoreElimintaUtenteTB() {
		getMapUtenti();
		when(ricDao.insert(any())).thenReturn(1);
		when(ricDao.delete(1)).thenReturn(false);
		when(utenteDao.getAll()).thenReturn(arr);
	...
	@Test
	public void gestoreUpdateUtenteTB() {
		getMapUtenti();
		when(ricDao.insert(any())).thenReturn(1);
		when(ricDao.delete(1)).thenReturn(false);
		when(utenteDao.getAll()).thenReturn(arr);
